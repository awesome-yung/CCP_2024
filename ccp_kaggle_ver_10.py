...
wandb.init(entity='2024CCP', project='yumin', name='distillation_1000epoch')
...
# Training fuction
def train(model,train_dataloader,device,optimizer,scheduler, epoch, EPOCHS, writer, train_loss_meter, criterion):
    ti = 0
    model.train()
...
def main(learning_rate, batch_size, epochs, num_workers, data_path, model_save_path, tensorboard_path, num_classes, checkpoint_path):
    ...
    child_model_pth = './working/trained_model/segformer_transit.h5'
    checkpoint_ = torch.load(child_model_pth)
    model = Segformer_b3()
    ...
    # Early Stop
    es = EarlyStopping(patience=1000,restore_best_weights=False)
    ...
    for epoch in range(current_epoch,epochs): #EPOCHS
        ...
        train_loss = train(model,train_dataloader,device, optimizer, scheduler, epoch, epochs, writer, train_loss_meter,criterion)
